...
The 14% capability anchor, preset 2030 endpoints, and labor-market calibration use `PAPER` or `DATA` values. The 100% capability ceiling, one-pool task-to-job mapping, reinterpretation of search effectiveness and posting speed, continuation of the paper's linear log-productivity path through 2040, smooth labor-flow threshold, and fixed 2040 interface horizon are `ASSUMPTION` values or rules. Job-loss pass-through does not appear in the paper. It replaces Equation (13)'s fixed-total, two-group target allocation in the one-pool extension. Its preset values are `CALCULATED` to reproduce Table 3's 2030 aggregate unemployment rates, then held constant through 2040 as an `ASSUMPTION`. Beginning after 2030, the non-passed-through share of incremental displacement is treated as replacement human-task demand in the production block; this is also an `ASSUMPTION`. Monthly chart values and labor income per labor-force participant are `CALCULATED` outputs.
