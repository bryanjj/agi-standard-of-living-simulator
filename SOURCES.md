...
The 14% capability anchor, preset 2030 endpoints, two-group employment shares, separation-rate relatives, wage rigidity, and labor-market calibration use `PAPER` or `DATA` values. The 100% capability ceiling, smooth transition that extends exposure into other work, continuation of the paper's linear log-productivity path through 2040, smooth labor-flow threshold, and fixed 2040 interface horizon are `ASSUMPTION` values or rules. Monthly chart values are `CALCULATED` outputs.
