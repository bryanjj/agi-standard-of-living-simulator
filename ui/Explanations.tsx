...
export function WhyChart({ year, scale = 1 }: { year: SimulationYear; scale?: number }) {
  const values = (Object.entries(year.decomposition) as [keyof typeof contributionLabels, number][]).map(([key, value]) => [key, value * scale] as const);
  ...
  return (
    <div className="why-chart">
      ...
      <div className="reconcile"><span>Today</span><strong>{idx(100 * scale)}</strong><span>+ contributions</span><strong>= {idx(year.standardOfLiving * scale)}</strong></div>
    </div>
  );
}
...
export function Affordability({ year }: { year: SimulationYear }) {
  ...
  return (
    <div className="afford-grid">
      ...
      <article><p><Term note="scarceFactors">IRREPRODUCIBLE SCARCE FACTORS</Term></p><strong>{idx(housing)}</strong><span>Today = 100</span><div className="meter scarce"><i style={{ width: `${Math.min(100, housing)}%` }} /></div><small>Examples include land and natural resources. Housing exposure is this simulator&apos;s household-level proxy.</small></article>
    </div>
  );
}
